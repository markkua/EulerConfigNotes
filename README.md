...
1. Establish an ssh tunnel by submitting a job (so you would not use the resource of login node)
	...
	cat $HOME/VSCode_tunnel
	code-server --bind-addr=${ip}:${port}
	```
	
    b. Submit a job to start the tunnel:
	...
	bsub -n 1 -W 4:00  bash  ~/PATH/TO/YOUR/SCRIPT
	```
	
    c. When the tunnel has **started** (job running), print the command by:

	```bash
	cat ~/VSCode_tunnel
	```

2. (On local machine) copy the command generated above and run it in **local bash**
...
4. (After) Kill the job on Euler. Then clean up ssh tunnel locally: 
